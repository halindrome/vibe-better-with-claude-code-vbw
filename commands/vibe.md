...
- **B6: CLAUDE.md** -- Read root CLAUDE.md. If it exists AND contains non-VBW content (sections not generated by VBW): preserve all existing content, replace only VBW-managed sections (## VBW Rules, ## Active Context, ## Key Decisions, ## Installed Skills, ## Project Conventions, ## Commands, ## Plugin Isolation). If no existing file: generate fresh with `# {project-name}` + `**Core value:** {one-liner}` + sections (Active Context, Key Decisions, Installed Skills, Project Conventions, Learned Patterns, VBW Commands). Max 200 lines. Regenerate on each update — always preserve non-VBW content.
