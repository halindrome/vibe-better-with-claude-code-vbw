...
```
Use **Workflow executor mode** (path 4 below) for that segment **only when ALL** of the following hold; otherwise fall through to the segment's resolver-assigned path (**True team mode**, path 1, for a `team` segment; **Explicit non-team mode**, path 2, for a `subagent` segment):
...
2. **Explicit non-team mode**
   ...
   - A `subagent` segment with `plan_ids` length ≥ 2 is first offered to the Workflow executor evaluation above; it lands here only when that returned `fallback` (e.g. `prefer_workflows=never`, runtime unsupported, or an interleave gate on).
