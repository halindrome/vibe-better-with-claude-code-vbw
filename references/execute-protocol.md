...
```
Use **Workflow executor mode** (path 4 below) for that segment **when `WF_EXECUTOR=workflow`**; otherwise fall through to the segment's resolver-assigned path (**True team mode**, path 1, for a `team` segment; **Explicit non-team mode**, path 2, for a `subagent` segment):
- `WF_EXECUTOR=workflow`. Driven by `prefer_workflows` (default `auto`, mirrors `prefer_teams`): the executor returns `workflow` when the runtime supports workflows (`WF_SUPPORTED=true` — the user's Claude config must enable them) **and** the segment has real parallel delegate work (`plan_ids` length ≥ 2). `prefer_workflows=never` forces `fallback`. There is no fan-out threshold beyond width ≥ 2 — a workflow fires at the same width a team would.
- **All per-task/per-plan gates are honored *within* the workflow, not by blocking it** — no gate forces a fallback. `validation_gates` is applied **pre-dispatch** (per-plan risk → approval/`qa_tier`/comms policy, resolved around the workflow), and `lease_locks` **and** `two_phase_completion` are relocated **into each worker's own task loop** (worker-self-lease and worker-self two-phase — see path 4). Both relocations are sound because the gate scripts are **deterministic and depend on no orchestrator-only state** — `lease-lock.sh` is a shared-filesystem lock and `two-phase-complete.sh` reads only the contract sidecar + committed files/git, so a worker running them after each of its own commits produces the **same verdict** the orchestrator would. The only conditions that fall back to the team/subagent path are `prefer_workflows=never`, an unsupported runtime, or fan-out < 2. (`PreToolUse`/`PostToolUse` hooks fire **inside** workflow workers; `SubagentStart`/`SubagentStop` do **not**, so each worker self-registers as an active VBW agent — see **Worker-self-registration** in path 4.)

This is the **predicate/owner split** applied to Execute: `resolve-execute-delegation-mode.sh` still owns team/subagent/direct selection and wave grouping unchanged; `resolve-executor.sh` is the orthogonal workflow-or-not predicate layered on top, driven by `prefer_workflows`. **`prefer_workflows` is evaluated before `prefer_teams`**: for any parallel-eligible segment (`plan_ids` length ≥ 2 — `team` **or** `subagent`), the executor is consulted first; on `workflow` it replaces that segment's team-or-serialized-subagent execution with a workflow. It falls through to the `prefer_teams` resolution (True team mode vs. serialized subagent) **only** when the executor returns `fallback` — i.e. `prefer_workflows=never`, the runtime is unsupported, or fan-out < 2. With the default `prefer_workflows=auto`, the workflow is the **preferred** parallel backend over Agent Teams whenever the runtime supports it.
...
2. **Explicit non-team mode**
   ...
   - A `subagent` segment with `plan_ids` length ≥ 2 is first offered to the Workflow executor evaluation above; it lands here only when that returned `fallback` (e.g. `prefer_workflows=never`, runtime unsupported, or fan-out < 2).
...
4. **Workflow executor mode (opt-in — Dynamic Workflows executor; team alternative)**
   - Use this path for a delegate segment (`delegation_mode` `team` **or** parallel `subagent`) when the Workflow executor evaluation above selected it (see precedence rule). It replaces that segment's True team mode **or** serialized-subagent execution with a Claude Dynamic Workflow dispatched via the first-class `Workflow` tool (see `${VBW_PLUGIN_ROOT}/references/workflow-executor.md` "Invoking a workflow"); the per-request `ultracode` keyword is the degraded alternate when the tool is absent.
   ...
   - **Validation-gate policy (pre-dispatch — applies when `validation_gates=true`):** resolve each delegate plan's gate policy (`assess-plan-risk.sh` → `resolve-gate-policy.sh`) **before** dispatch, exactly as the team/subagent paths do. Apply `approval_required` as a plan-approval step **before** the worker is dispatched (a background worker cannot do interactive plan-mode approval); feed `qa_tier` into the Step 4 QA that runs after the segment; teammate `communication_level` is not applicable to workflow workers. If the resolved policy sets `two_phase=true` for any plan, that plan stays workflow-eligible — its worker runs the per-task two-phase loop itself (see **Worker-self two-phase** below).
   - **Worker-self-registration (ALWAYS under the workflow executor — not gated by a config flag):** the `SubagentStart`/`SubagentStop` hooks that normally register a subagent as an active VBW agent **do not fire for `Workflow`-tool-spawned workers** (only for Task/Team subagents). Without registration, `file-guard.sh` reads a per-session active-agent count of `0`, misclassifies each worker as the orchestrator, and blocks **all** its product-file Write/Edit (`orchestrator cannot write product files during delegated workflow`). The executor therefore injects a registration call into each worker's prompt: run `bash "${VBW_PLUGIN_ROOT}/scripts/workflow-worker-register.sh" register {SESSION_ID} dev {NN}-{MM}` **before the worker's first file write**, and `bash "${VBW_PLUGIN_ROOT}/scripts/workflow-worker-register.sh" release {SESSION_ID} dev {NN}-{MM}` **at the very end** (after the final commit). Substitute `{SESSION_ID}` with the orchestrator's resolved session id (`$CLAUDE_SESSION_ID` — the same id in the `/tmp/.vbw-plugin-root-link-<id>` symlink); the script registers into exactly the per-session bucket `file-guard.sh` consults (`.active-agents/<session_id>/active-agent-count`), never the aggregate `__vbw_legacy_global` bucket the guard ignores. Both are **single bounded commands** (no pipes/redirects) so consumer `PreToolUse` hooks do not block them mid-run. This is the registration analogue of worker-self-lease/two-phase: an enforcement signal the orchestrator would normally own via subagent hooks, relocated into the worker because the workflow lifecycle skips those hooks.
   - **Worker-self-lease (applies when `lease_locks=true`):** instruct each worker, in its prompt, to acquire an exclusive file lease on its plan's claimed files before its first write and release it when done — `bash "${VBW_PLUGIN_ROOT}/scripts/lease-lock.sh" acquire {NN}-{MM} --ttl=300 {claimed_files...}` at start, `bash "${VBW_PLUGIN_ROOT}/scripts/lease-lock.sh" release {NN}-{MM}` at end, renewing on long tasks. `lease-lock.sh` is a shared-filesystem lock, so concurrent workflow workers are mutually excluded on overlapping files exactly as Agent-Team members are; combined with the structural guarantee (one worker per independent plan, tasks sequential within a worker) this preserves `lease_locks` semantics under the workflow executor. The `file-guard.sh` PreToolUse check still fires inside workers regardless.
   - **Worker-self two-phase (applies when `two_phase_completion=true`):** instruct each worker, in its prompt, to run the per-task candidate→confirmed gate on **its own** commits — after each task commit, `RESULT=$(bash "${VBW_PLUGIN_ROOT}/scripts/two-phase-complete.sh" {task_id} {NN} {MM} {contract_path} {evidence...})`. On `result=confirmed`, proceed to the next task; on `result=rejected`, attempt bounded self-repair (re-run the task's checks / fix, then re-run two-phase) **at most twice**, and if still rejected, stop, set the plan's `status=partial|failed` with the rejection captured in `deviations`, and return — do **not** loop indefinitely (a worker cannot escape its own blind spot; the orchestrator's coverage screen + Step 3c catch the returned failure and re-dispatch or escalate). This preserves two-phase's verdict under workflows because `two-phase-complete.sh` is deterministic and orchestrator-state-free (reads the contract sidecar + committed files); the candidate/confirmed events are emitted by the worker exactly as the orchestrator would. The genuinely *independent* verification (semantic, goal-backward) remains the separate QA phase and the orchestrator's Step 3c SUMMARY.md gate — both unchanged.
   - **Dispatch** `Workflow({ script })` with an inline script that runs the segment's delegate plans as workers, capped to `${WF_MAX_WORKERS}` concurrent (`0` = no VBW cap; the runtime still caps at 16 concurrent / 1000 total):
     - For each delegate plan in the segment, spawn one worker via `agent(devPrompt, { agentType: 'vbw:vbw-dev', model: '${DEV_MODEL}', schema: <plan-summary-schema> })`. Segments are already a single dependency wave of independent plans, so the plans are parallel-safe. **Honor `${WF_MAX_WORKERS}` as a concurrency cap, not a total cap:** there is one worker per plan (so N plans ⇒ N workers total), but at most `${WF_MAX_WORKERS}` may run **at once**. `parallel()` has **no** concurrency argument — passing all plans to a single `parallel()` call ignores the VBW cap and runs up to the runtime's own ceiling (16). To honor the cap, **chunk the plans into groups of `${WF_MAX_WORKERS}` and `await parallel()` on each group in turn** (when `${WF_MAX_WORKERS}=0`, no VBW cap — one `parallel()` over all plans, bounded only by the runtime). Example: `for (const g of chunk(plans, WF_MAX_WORKERS)) { await parallel(g.map(p => () => agent(...))) }`.
     - Each worker MUST: execute all tasks in its `{NN}-{MM}-PLAN.md`, commit per task (git commits are not gated artifacts), run any enabled per-task gates on its own commits (worker-self-lease and worker-self two-phase, per the bullets above), and **return** structured per-plan result via the schema — `status` (`complete|partial|failed`), `commit_hashes`, `files_modified`, `tasks_completed`, `tasks_total`, `deviations`, `must_haves_met`, and `tooling_unavailable` (true when the worker could not execute because tools/Bash/filesystem/edit access were missing or a `PreToolUse` gate blocked it — an environment block, distinct from a real task failure). Workers MUST NOT write `{NN}-{MM}-SUMMARY.md` — it is a gated artifact and the orchestrator bridges it.
   ...
   - **Coverage + provisioning screen (mandatory — never infer success from absence of failure):** before bridging, screen every returned plan result. **(a) Tool/provisioning block:** if a result has `tooling_unavailable=true`, or `status=failed` with evidence that tools/Bash/filesystem/edit/API access were unavailable (mirror the non-team **Subagent return handling** below), treat it as a platform/tool provisioning blocker — do **not** count it as a completed plan, do **not** consume the normal retry budget, and surface it; re-dispatch only after the environment is fixed. **(b) Empty/zero work:** a result with empty `commit_hashes` **and** `tasks_completed=0` (or `tasks_total=0`) is a **failure-to-execute**, not a pass — a worker that did nothing and a worker that finished are indistinguishable by `status` alone (observed in dogfooding: silent no-op workers reported zero failures and looked green). Confirm every expected delegate plan returned a non-empty, executed result before marking the segment done; any plan with no worker result, an empty/zero result, or a provisioning block is re-dispatched (workflow if still eligible, else its resolver-assigned path) up to the same retry budget as the non-team path, then escalated to the user. A `parallel()`/`pipeline()` slot that dropped to `null` (worker died) counts as no result and is re-dispatched, never silently skipped.
   ...
   - **Fallback (mandatory):** if the `Workflow` tool is unavailable at dispatch, the run is refused/errors, or it returns no usable result, fall back to the segment's resolver-assigned path: for a `team` segment use **True team mode** (path 1; if real team semantics are unavailable, the team-tooling-unavailable fallback, path 3); for a `subagent` segment use **Explicit non-team mode** (path 2, serialized subagent). Backward-compat is preserved by the two-half gate: the workflow path runs only when the runtime supports workflows (the user's Claude config enables them) **and** `prefer_workflows≠never`; every fallback lands exactly where today's routing would have put the segment.
...
- **On the Workflow executor path (path 4):** this per-task loop is run **by each worker on its own commits** (worker-self two-phase), not by the orchestrator — `two-phase-complete.sh` is deterministic and orchestrator-state-free, so the verdict is identical. The orchestrator does **not** interleave it for workflow-dispatched plans; it relies on the worker's bounded self-repair plus the Step 3c SUMMARY.md gate and coverage screen on the returned result.
