...
if [ ! -f "$CACHE" ] || [ $((NOW - MT)) -gt 86400 ]; then
  ...
  if [ -n "$REMOTE_VER" ] && [ "$REMOTE_VER" != "0.0.0" ] && [ "$REMOTE_VER" != "$LOCAL_VER" ]; then
    UPDATE_MSG="${UPDATE_MSG} UPDATE AVAILABLE: v${LOCAL_VER} -> v${REMOTE_VER}. Run /vbw:update to upgrade."
  fi
else
  # Read cached result
  ...
  if [ -n "${REMOTE_VER:-}" ] && [ "${REMOTE_VER:-}" != "0.0.0" ] && [ "${REMOTE_VER:-}" != "${LOCAL_VER:-}" ]; then
    UPDATE_MSG="${UPDATE_MSG} UPDATE AVAILABLE: v${LOCAL_VER:-0.0.0} -> v${REMOTE_VER:-0.0.0}. Run /vbw:update to upgrade."
  fi
fi
  ...
  if [ "$exec_running" = true ]; then
    ...
    case "$PD_NEXT_PHASE_STATE" in
      ...
      needs_reverification)
        NEXT_ACTION="/vbw:vibe (Phase ${PD_NEXT_PHASE} remediation complete, run re-verification)"
        ;;
      needs_verification)
        NEXT_ACTION="/vbw:vibe (Phase ${PD_NEXT_PHASE} needs UAT verification)"
        ;;
      needs_discussion)
